...
public class QMCDecoder {
    private var isOptimiseCrypto: Bool
    ...
    let privateKey256: [UInt8] = [0x77, 0x48, 0x32, 0x73, 0xDE, 0xF2, 0xC0, 0xC8, 0x95, 0xEC, 0x30, 0xB2, 0x51, 0xC3, 0xE1, 0xA0,
    ...
    init(isOptimiseCrypto: Bool = true) {
        self.isOptimiseCrypto = isOptimiseCrypto
    }
    
    func qmcCryptoTransform(data: Data, offset: Int, size: Int) -> Data {
        var resultArray = [UInt8](repeating: 0, count: size)
        for (index, byte) in data.enumerated() {
            resultArray[index] = byte ^ qmcCryptoEncode(index: offset + index)
        }
        return Data(resultArray)
    }
    
    func qmcCryptoEncode(index: Int) -> UInt8 {
        if isOptimiseCrypto {
            if index > 0x7FFF {
                return privateKey128[(index % 0x7FFF) & 0x7F]
            } else {
                return privateKey128[index & 0x7F]
            }
        } else {
            let offset = index > 0x7FFF ? (index % 0x7FFF) : index
            let index = (offset * offset + 27) & 0xFF
